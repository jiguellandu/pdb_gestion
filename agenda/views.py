...
def liste_rendez_vous(request):
    ...
    if user.role == "administrateur":
        ...
        rendez_vous = (
            RendezVousPasteur.objects
            .filter(eglise=user.eglise, pasteur=user)
            .order_by("date", "heure_debut")
        )
...
def creer_rendez_vous(request):
    ...
    if user.role not in ("administrateur", "pasteur"):
        messages.error(
            request,
            "Tu n'as pas le droit de creer un rendez-vous."
        )
    ...
    if request.method == "POST":
        ...
        if user.role == "pasteur" and "pasteur" in form.fields:
            form.fields.pop("pasteur")

    return render(
        request,
        "agenda/creer_rendez_vous.html",
        {"form": form}
    )
...
def creer_evenement(request):
    if request.user.role != "administrateur":
        messages.error(
            request,
            "Seule l'administration peut creer un evenement."
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            evenement.eglise = request.user.eglise
            evenement.save()
